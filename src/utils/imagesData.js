// export const heroImages = [
//   'https://i.ibb.co/P14N6dQ/igor-karimov-Yuipf-Pt-OH1k-unsplash.jpg',
//   'https://i.ibb.co/1Rs7mPF/venson-chou-n-D2-Bj-SLt0u-E-unsplash.jpg',
//   'https://i.ibb.co/WDC9GVc/akhil-yerabati-Q2u-V5-Tkj-Nz8-unsplash.jpg',
//   'https://i.ibb.co/3F7W664/jeshoots-com-e-Cktz-Gj-C-i-U-unsplash.jpg',
// ];

import pic0 from '../assets/0.jpg';
import pic1 from '../assets/1.jpg';
import pic2 from '../assets/2.jpg';
import pic3 from '../assets/3.jpg';

export const heroImages = [pic0, pic1, pic2, pic3];

export const featuredConsoles = [
  'https://i.ibb.co/0MP2BCL/playstation-5.jpg',
  'https://i.ibb.co/nnpNWDc/xbox.jpg',
];

export const productsImages = [
  { name: 'gaming consoles', url: 'https://i.ibb.co/ykgNs1s/steam-deck.jpg' },
  {
    name: 'controllers',
    url: 'https://i.ibb.co/g6fGySn/charindra-salgado-8-Kz-AAf4-GKuk-unsplash.jpg',
  },
  {
    name: 'headphones',
    url: 'https://i.ibb.co/TtkbtfR/alexunder-hess-b-WZAPKm0z-ZE-unsplash.jpg',
  },
  {
    name: 'keyboards',
    url: 'https://i.ibb.co/rZMTK96/jay-zhang-ZBy-Wa-PXD2f-U-unsplash.jpg',
  },
  {
    name: 'monitors',
    url: 'https://i.ibb.co/2PPpXQf/nicolas-gonzalez-183-F-v-Na-N8-A-unsplash.jpg',
  },
  {
    name: 'mouse',
    url: 'https://i.ibb.co/y0dFwVt/shagal-sajid-Ox7m-Smsljkc-unsplash.jpg',
  },
];
